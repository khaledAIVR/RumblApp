defmodule Rumbl.MultimediaTest do
  ...
  describe "videos" do
    ...
    @valid_attrs %{description: "desc", url: "http://local"}
    ...
    test "get_video!/1 returns a video with the given id" do
      owner = user_fixture()
    ...
    test "create_video/2 with invalide data returns error changeset" do
      owner = user_fixture()
    ...
    test "update_video/2 with valid data updates a video" do
      ...
      assert {:ok, video} = Multimedia.update_video(video, %{description: "updated desc"})
      assert %Video{} = video
