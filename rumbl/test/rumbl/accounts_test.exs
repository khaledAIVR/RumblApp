defmodule Rumbl.AccountTest do
  ...
  describe "register_user/1" do
    ...
    test "with valid data, insert a user" do
    ...
    test "with invalid data doesn't insert a user" do
    ...
    test "requires password to be at least 6 chars long" do
      assert attrs = Map.put(@valid_attrs, :password, "12345")
      assert {:error, changeset} = Accounts.register_user(attrs)
  ...
  describe "authenticate_by_username_and_pass/2" do
    ...
    test "returns not found error with no matching user for email" do
      assert {:error, :not_found} =
               Accounts.authenticate_by_username_and_password("unknown", @pass)
    end
  end
end
